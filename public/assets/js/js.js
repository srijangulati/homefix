...
function parseJSON(str){
	...
	/*$("a img.image-hover").on("each", function(){
			alert($(this).attr("data-hover-src"));
		$(this).on("click", function(){
			alert($(this).attr("data-hover-src"));
			if($(this).attr("data-hover-src") !== '') {
...
$(function(){ 
	function load_services(){
		...
		$.ajax({
		url: apiurl + "/request_services",
		  type: 'POST',
		  dataType:"html",
		  data: {},
		  error: function( response ) {
			  hideIndicator();
			  alert("An error occurred. Please contact us is problem persists.<br />" + response.statusText);
			},
		  success: function (data) {
			  ...
			  var Data = parseJSON(data);
				if(Data.status.trim() === 'success'){
					...
					var sevicesModal = '<div id="requestServicesBox" class="w3-modal">' +
								'<div class="w3-modal-content w3-border" style="margin-top: 20px; margin-bottom: 20px;">' +
								  '<header class="w3-container w3-red w3-padding"> ' +
									'<span onclick="document.getElementById(\'requestServicesBox\').style.display=\'none\'" class="w3-button w3-display-topright">&times;</span>' +
									'<h5 id="request-title" class="bold w3-text-white">SERVICES</h5>' +
								   '</header>' +
								  '<div class="" style="padding: 20px 30px;">' + result +
								  '</div>' +
							   '</div>' +
							  '</div>';
							  
						$("body").find("#requestServicesBox").remove();
						$("body").append(sevicesModal);
						$("body").find("#requestServicesBox").show();
